...
class CervixMobileODT(SampledDataset):
    K = NUM_CLASSES
    ...
    def __init__(self, root, split, dataset_scale, transform=None):
        ...
        root = os.path.join(root, 'CervixMobileODT', 'subset')
...
class CervixMobileODT_DP(DatasetProvider):
    def __init__(self, scale: float = 1, mask_type: str = None):
        print("WARNING: 'mask_type' parameters aren't being used in CervixMobileODT dataset!")
    ...
    def create_train(self):
        return CervixMobileODT(ROOT_PATH, 'train', dataset_scale=self.scale, transform=self.transf)

    def create_val(self):
        return CervixMobileODT(ROOT_PATH, 'train', dataset_scale=self.scale, transform=self.test_transf)

    def create_test(self):
        return CervixMobileODT(ROOT_PATH, 'test', dataset_scale=self.scale, transform=self.test_transf)
