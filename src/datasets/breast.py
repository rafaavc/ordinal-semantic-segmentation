...
class Breast(SampledDataset):
    K = NUM_CLASSES
    
    def __init__(self, root, split, dataset_scale: float, transform=None):
        super().__init__()
        assert split in ('train', 'test')
        self.transform = transform

        root = os.path.join(root, 'breast-aesthetic')
        self.imgs_dir = os.path.join(root, 'imgs')
        self.masks_dir = os.path.join(root, 'masks')

        files = [f.removesuffix('.jpg').removesuffix('.JPG') for f in os.listdir(self.imgs_dir) if f.lower().endswith('.jpg')]
...
class Breast_DP(DatasetProvider):
    def __init__(self, scale: float = 1, mask_type: str = None):
        print("WARNING: 'mask_type' parameters aren't being used in Breast dataset!")
    ...
    def create_train(self):
        return Breast(ROOT_PATH, 'train', dataset_scale=self.scale, transform=self.transf)

    def create_val(self):
        return Breast(ROOT_PATH, 'train', dataset_scale=self.scale, transform=self.test_transf)

    def create_test(self):
        return Breast(ROOT_PATH, 'test', dataset_scale=self.scale, transform=self.test_transf)
