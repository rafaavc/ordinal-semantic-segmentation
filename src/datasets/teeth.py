...
class Teeth_DP(DatasetProvider):
    def __init__(self, scale: float = 1, mask_type: str = None):
        print("WARNING: 'mask_type' parameters aren't being used in Teeth dataset!")
    ...
    def create_train(self):
        return Teeth(ROOT_PATH, 'train', dataset_scale=self.scale, transform=self.transf)

    def create_val(self):
        return Teeth(ROOT_PATH, 'train', dataset_scale=self.scale, transform=self.test_transf)

    def create_test(self):
        return Teeth(ROOT_PATH, 'test', dataset_scale=self.scale, transform=self.test_transf)
